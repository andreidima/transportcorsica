...
class PlataOnlineController extends Controller
{
    ...
    {
        $this->paymentUrl   = env('MOBILPAY_PAYMENT_URL', '');
        $this->x509FilePath = env('MOBILPAY_PUBLIC_KEY_PATH', '');
        try {
            $paymentRequest = new Card();
            $paymentRequest->signature  = env('MOBILPAY_MERCHANT_ID', '');//signature - generated by mobilpay.ro for every merchant account
            $paymentRequest->orderId    = md5(uniqid(rand())); // order_id should be unique for a merchant account
            $paymentRequest->confirmUrl = env('MOBILPAY_CONFIRM_URL', ''); // is where mobilPay redirects the client once the payment process is finished and is MANDATORY
            $paymentRequest->returnUrl  = env('MOBILPAY_RETURN_URL', '');;// is where mobilPay will send the payment result and is MANDATORY
            ...
            return Redirect::away($this->paymentUrl)->with(['EnvKey'=>$EnvKey, 'data'=>$data]);
